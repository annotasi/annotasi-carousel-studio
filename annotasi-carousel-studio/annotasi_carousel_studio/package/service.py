...
def package_required_issues(record: dict[str, Any]) -> list[str]:
    ...
    if not completed_png_for_package(record):
        issues.append("PNG carousel render not found. Run /render first.")
...
def ready_to_post_check(item_id: str) -> dict[str, Any]:
    ...
    png_stale_blocked = bool(isinstance(stale, dict) and stale.get("png") and not CONTENT_PACKAGE_ALLOW_STALE_MEDIA)
    if png_stale_blocked:
        required.append("PNG render is stale. Please rerun /render before packaging.")
    missing = [
        item for item in warnings
        if ("PNG" in item and "not found" in item)
        or ("title" in item.lower() and "missing" in item.lower())
        or ("caption" in item.lower() and "missing" in item.lower())
        or ("hashtags" in item.lower() and "missing" in item.lower())
    ]
...
def generate_content_package(item_id: str, body: dict[str, Any]) -> dict[str, Any]:
    ...
    required = package_required_issues(record)
    stale = ensure_workflow(record).get("renderStale", {})
    if isinstance(stale, dict) and stale.get("png") and not CONTENT_PACKAGE_ALLOW_STALE_MEDIA:
        required.append("PNG render is stale. Please rerun /render before packaging.")
    if required:
